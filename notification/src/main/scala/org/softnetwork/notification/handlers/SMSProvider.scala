...
trait SMSModeProvider extends SMSProvider {
  ...
  import org.softnetwork.notification.config.SMSMode._
  import Status._
  ...
  lazy val config = Settings.config match {
    case Some(conf) => conf.sms.mode
    case None => None
  }

  override def send(notification: SMS): NotificationAck = {
  ...
  override def ack(notification: SMS): NotificationAck = {
