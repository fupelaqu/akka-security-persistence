...
import org.softnetwork.akka.message.{EntityCommand, CommandResult, Command}
...
trait EntityHandler[C <: Command, R <: CommandResult] extends StrictLogging {_: CommandTypeKey[C] =>
  ...
  private[this] def entityRef(entityId: String)(implicit system: ActorSystem[_], tTag: ClassTag[C]) =
  ...
  private[this] def tell(entityId: String, command: C)(implicit system: ActorSystem[_], tTag: ClassTag[C]): Unit = {
  ...
  def !!(command: C)(implicit system: ActorSystem[_], tTag: ClassTag[C]): Unit =
    command match {
