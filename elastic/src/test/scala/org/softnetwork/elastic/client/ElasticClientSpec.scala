...
import org.softnetwork.akka.model.{Sample, Timestamped}
import org.softnetwork.akka.serialization._
import org.softnetwork.elastic.test.ElasticDockerTestKit
import org.softnetwork.elastic.utils._
...
/**
  ...
  */
class ElasticClientSpec extends FlatSpec with TestSuite with ElasticDockerTestKit {

  implicit val system = ActorSystem("elasticActor")

  implicit val executionContext = system.dispatcher

  implicit val formats = commonFormats

  val pClient = new PersonApi(esCredentials)
  val sClient = new SampleApi(esCredentials)
  val bClient = new BinaryApi(esCredentials)
...
//    "person2" should haveCount(5)
  ...
  "Count" should "work" in {
    ...
    val response = pClient.countAsync("{}", Seq[String]("person6"), Seq[String]()).await
  ...
  "Search" should "work" in {
    ...
    val response = pClient.searchAsync("select * from person7").await

    response.isDefined shouldBe true
    response.get.getTotal should ===(3)

    val response2 = pClient.searchAsync("select * from person7 where _id=\"A16\"").await

    response2.isDefined shouldBe true
    response2.get.getTotal should ===(1)
...
class PersonApi(credentials: ElasticCredentials) extends ElasticApi[Person] with ManifestWrapper[Person]{
  override protected val manifestWrapper: ManifestW = ManifestW()
  implicit lazy val jestClient: JestClient = apply(credentials, multithreaded = false)
}

class SampleApi(credentials: ElasticCredentials) extends ElasticApi[Sample] with ManifestWrapper[Sample]{
  override protected val manifestWrapper: ManifestW = ManifestW()
  implicit lazy val jestClient: JestClient = apply(credentials, multithreaded = false)
}

class BinaryApi(credentials: ElasticCredentials) extends ElasticApi[Binary] with ManifestWrapper[Binary]{
  override protected val manifestWrapper: ManifestW = ManifestW()
  implicit lazy val jestClient: JestClient = apply(credentials, multithreaded = false)
}
