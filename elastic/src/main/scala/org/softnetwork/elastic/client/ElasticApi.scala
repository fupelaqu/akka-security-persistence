...
import org.softnetwork.elastic.config.Settings
import org.softnetwork.elastic.config.Settings.ElasticConfig
...
import io.searchbox.indices.{Flush, Refresh, CreateIndex, DeleteIndex}
    ...
    jestClient match {
      ...
      case None =>
        httpClientConfig = new HttpClientConfig.Builder(esCredentials.url)
          ...
          .preemptiveAuthTargetHosts(getHttpHosts(esCredentials.url))
...
trait IndicesApi {_: JestClientCompanion =>
  def createIndex(index: String) = apply().execute(new CreateIndex.Builder(index).build()).isSucceeded
...
trait UpdateSettingsApi {_: JestClientCompanion =>
...
trait IndexApi {_: JestClientCompanion =>
  def index[U <: Timestamped](entity: U, index: Option[String] = None, `type`: Option[String] = None)(
    ...
    val _type  = `type`.getOrElse(manifest[U].runtimeClass.getSimpleName.toLowerCase)
...
trait DeleteApi {_: JestClientCompanion =>
  ...
  def deleteAsync[U <: Timestamped](entity: U, index: Option[String] = None, `type`: Option[String] = None)(
    implicit m: Manifest[U], ec: ExecutionContext): Future[Boolean] = {
      ...
      val result = b.add(
        Flow[BulkResult]
          ...
          .map(result => {
            ...
            val indices = items.map(_.index).toSet
  ...
  def get[U <: Timestamped](id: String, index: Option[String] = None, `type`: Option[String] = None)(
    ...
    val result = apply().execute(
      new Get.Builder(
        index.getOrElse(
          `type`.getOrElse(
            manifest[U].runtimeClass.getSimpleName.toLowerCase
          )
        ),
        id
      ).build()
...
trait SearchApi {_: JestClientCompanion =>
    ...
    (for(result <- results)
      yield
        (
          ...
          (for(innerHit <- innerHits(result)) yield
            innerHit match {
              case obj: JsonObject =>
                Try{
  ...
  def multiSearch(sqlQueries: List[String]): Option[MultiSearchResult] = {
    ...
    if(searches.size == sqlQueries.size){
      Some(apply().execute(new MultiSearch.Builder(searches).build()))
    }
    else{
      None
    }
  ...
  def multiGetAll[U](sqlQueries: List[String])(implicit m: Manifest[U], formats: Formats): List[List[U]] = {
    multiSearch(sqlQueries) match {
      case Some(multiSearchResult) =>
        multiSearchResult.getResponses.map(searchResponse =>
  ...
  def nativeMultiGetAllWithInnerHits[U, I](searches: List[Search], field: String)(
    implicit m1: Manifest[U], m2: Manifest[I], formats: Formats): List[List[(U, List[I])]] = {
    val multiSearchResult = apply().execute(new MultiSearch.Builder(searches).build())
...
trait JestProvider[T <: Timestamped] extends ElasticProvider[T] {_: ElasticApi[T] with ManifestWrapper[T] =>
  ...
  /**
    ...
    */
  override def createDocument(document: T)(implicit m: Manifest[T]): Boolean = {
  ...
  /**
    ...
    * @return whether the operation is successful or not
    */
  override def updateDocument(document: T, upsert: Boolean)(implicit m: Manifest[T]): Boolean = {
    implicit val jestClient = apply()
  ...
  /**
    ...
    */
  override def loadDocument(uuid: String): Option[T] = {
    implicit val jestClient = apply()
  ...
  /**
    ...
    */
  override def searchDocuments(query: String): List[T] = {
    implicit val jestClient = apply()
